...
EXTRA_SAMPLE_USER = {"name": "AppDev Premium",
                     "username": "appdevpremium", "password": "abc123"}
...
class TestRoutes(unittest.TestCase):
    ...
    def _send_money(self, user_id1, user_id2, amount, status_code, password=None, extra=False):
        ...
        res = requests.post(gen_send_path(extra=extra),
    ...
    def test_extra_get_user_unauthorized(self):
        ...
        res = requests.post(gen_users_path(user["id"], extra=True), data=json.dumps(
            {"password": "bad" + user.get('password')}))
    ...
    def test_extra_send_money(self):
        ...
        user_id2 = self._create_user_and_assert_balance(
            10, extra=True).get("id")
        self._send_money(user_id1, user_id2, 6, 200,
                         password=user1.get("password"), extra=True)
    ...
    def test_extra_send_money_unauthorized(self):
        ...
        user_id2 = self._create_user_and_assert_balance(
            10, extra=True).get("id")
        self._send_money(user_id1, user_id2, 6, 401,
                         password="bad" + user1.get("password"), extra=True)
